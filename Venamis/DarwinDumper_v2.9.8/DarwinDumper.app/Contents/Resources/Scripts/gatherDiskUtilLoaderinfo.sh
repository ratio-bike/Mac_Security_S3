...
Initialise()
{
    ...
    local scriptsDir="$( pwd )"/Scripts

    if [ "$2" == "0" ]; then
        gRunBootloaderConfigFiles=0
    else
        gRunBootloaderConfigFiles=1
        echo "S:diskLoaderConfigs" >> "$gDDTmpFolder"/dd_completed
    fi
    if [ "$3" == "0" ]; then
        gRunDiskutilAndLoaders=0
    else
        gRunDiskutilAndLoaders=1
        echo "S:bootLoaderBootSectors" >> "$gDDTmpFolder"/dd_completed
    fi
    if [ "$4" == "0" ]; then
        gRunDisks=0
    else
        gRunDisks=1
        echo "S:diskPartitionInfo" >> "$gDDTmpFolder"/dd_completed
    fi

    fdisk440="$toolsDir/fdisk440"
    bgrep="$toolsDir/bgrep"

    # Resources - Scripts
    bdisk="$scriptsDir/bdisk.sh"
...
DumpDiskUtilAndLoader()
{
    ...
    local diskUtilLoaderInfoFile="$gDDTmpFolder"/diskutilLoaderInfo.txt
    ...
    GrabXUIDs()
    {
        ...
        if [[ "$passedVolumeName" == "$gESPMountPrefix"* ]]; then
            local volumeNameToDisplay="EFI System Partition"
        else
    ...
    FindStringInExecutable()
    {
        ...
        local selection=$( tail -c +$((0x${offsetH##*: }+1)) "$passedFile" | head -c 128 | xxd -p | tr -d '\n' )
    ...
    CheckForBootFiles()
    {
        ...
        #Êignoring any boot* files with .extensions.
    ...
    FindAndCopyUserPlistFiles()
    {
        ...
        SaveFiles()
        {
            ...
            echo "${gLogIndent}Found ${passedFile}" >> "${gLogFile}"
            dirToMake="${passedFile%/*}"
            if [[ "$passedVolumeName" == *$gESPMountPrefix* ]]; then
                # Strip the preceding /Volumes/xxxxxxx/
                for (( x=1; x<=3; x++ ))
                do
                    dirToMake="${dirToMake#*/}"
                done
                dirToMake="${passedDevice}-EFI/${dirToMake}"
            else
            ...
            if [ -d "$gDumpFolderBootLoaderConfigs/$dirToMake" ]; then
                ...
                echo "${gLogIndent}Error: Failed to create directory: $gDumpFolderBootLoaderConfigs/$dirToMake" >> "${gLogFile}"
            fi
        }
    
        echo "${gLogIndent}Searching for Bootloader files on $passedDevice" >> "${gLogFile}"
        ...
        do
            if [ -d "${passedVolumeName}"/"${driverFolders[$q]}" ]; then
                if [[ "$1" == "$gESPMountPrefix"* ]]; then
                    dirToMake="${passedDevice}-EFI/EFI/${driverFolders[$q]}"
                else
    ...
    FindAndCopyRefitConfFile()
    {
        ...
        do
            ...
            if [ -f "${pathToSearch}"/refit.conf ]; then
                if [[ "$1" == "$gESPMountPrefix"* ]]; then
                    dirToMake="${passedDevice}-EFI${secondaryPaths[$q]}"
                else
    ...
    GetEspMountPoint()
    {
         ...
         # Add check to make sure disk1s10 is not used when looking for disk1s1
         local crossCheck=$( df -T hfs,msdos | grep "$passedDevice" )
         checkDevice="${crossCheck%% *}"
         checkMountPoint=$( echo "$crossCheck" | sed -ne 's/.*Volumes\///p' )
         
#echo "passedDevice=|${passedDevice}|  checkDevice=|${checkDevice}|   checkMountPoint=|${checkMountPoint}|"  

         if [ ! "$checkDevice" == "" ] && [ ! "$passedDevice" == "$checkDevice" ]; then
             checkMountPoint=""
         fi
...
#printf "C:$mountPoint\n"
    ...
    do
        ...
        echo "${gLogIndent}Scanning disk: ${duWholeDisks[$d]}" >> "${gLogFile}" # To logfile
        if [ $gRootPriv -eq 1 ]; then
        ...
        if [ $gRunDisks -eq 1 ]; then
            echo "${gLogIndent}Reading partition info for: ${duWholeDisks[$d]}" >> "${gLogFile}"
        ...
        do
            ...
            if [[ ${duIdentifier[$v]} == ${duWholeDisks[$d]}* ]] && [[ ! ${duIdentifier[$v]} == ${duWholeDisks[$d]} ]] ; then   
                ...
                # If we mounted an EFI system partition earlierÊthen we should un-mount it.
...
if [ $gRunBootloaderConfigFiles -eq 1 ]; then
    echo "F:diskLoaderConfigs" >> "$gDDTmpFolder"/dd_completed
fi
if [ $gRunDisks -eq 1 ]; then
    echo "F:diskPartitionInfo" >> "$gDDTmpFolder"/dd_completed
fi
