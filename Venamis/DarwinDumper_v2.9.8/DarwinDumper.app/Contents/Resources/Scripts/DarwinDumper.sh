...
#
# Re-structured and further developed by blackosx July 2012 -> Aug 2014
...
InitialiseBeforeUI()
{
    ...
    if [ -d "$resourcesDir" ]; then
        ...
        generateHTMLreportNew="$scriptsDir/generateHTMLreport_new.sh"
...
{        
    ...
    do
        ...
        case "${uiReturnArray[$x]##*:}" in
                    ...
                    "ReportNew")              gRadio_reportType="ReportNew"
                                              echo "*User Chose: New Report" >> "$gTmpPreLogFile" ;;
                    "ReportClassicCollapsed") gRadio_reportType="ReportClassicCollapsed"
                                              echo "*User Chose: Classic Report with collapsed option" >> "$gTmpPreLogFile" ;;
                    "ReportClassic")          gRadio_reportType="ReportClassic"
                                              echo "*User Chose: Classic Report" >> "$gTmpPreLogFile" ;;
                    "ReportNone")             gRadio_reportType="ReportNone"
...
DumpFilesFirmwareLog()
{
    ...
    if [ $fileSize -le 4 ]; then
        ...
        echo "${gLogIndent}Firmware Log not available."
        echo "${gLogIndent}Firmware Log not available." >> "${gLogFile}"
    else
        echo "Dumped ${gTheLoader} Firmware log."
...
--------------------------------------------------------" >> "${gLogFile}"
    ...
    if [ "$gRadio_reportType" == "ReportNew" ] || [ "$gRadio_reportType" == "ReportClassicCollapsed" ] || [ "$gRadio_reportType" == "ReportClassic" ]; then
        ...
        # Read tmp array for audio codecs and compile in to am html string for the report overview.                
        if [ ${#tmp[@]} -gt 0 ]; then
            gCodecID="${tmp[0]}"   
            for (( c=1; c<${#tmp[@]}; c++ ))
            do
                gCodecID="${gCodecID}<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"$( printf '%s\n\r' "${tmp[$c]##*: }" )
            done
        else
            gCodecID="${tmp[@]}"  
        fi
        ...
        if [ "$gRadio_reportType" == "ReportNew" ] || [ "$gRadio_reportType" == "ReportClassicCollapsed" ]; then
            tableState="none"
        else
            tableState="block"
        fi
        echo "S:htmlReport" >> "$gDDTmpFolder"/dd_completed
        # Build the html files.
        if [ "$gRadio_reportType" == "ReportClassicCollapsed" ] || [ "$gRadio_reportType" == "ReportClassic" ]; then
            "$generateHTMLreport" "$gMasterDumpFolder" "$gtheprog" "$gtheVersion" "$tableState" "$gCodecID" "$gRadio_privacy" "$gDataDir"
        elif [ "$gRadio_reportType" == "ReportNew" ]; then
            "$generateHTMLreportNew" "$gMasterDumpFolder" "$gtheprog" "$gtheVersion" "$tableState" "$gCodecID" "$gRadio_privacy" "$gDataDir"
        fi
        echo "F:htmlReport" >> "$gDDTmpFolder"/dd_completed
    fi
