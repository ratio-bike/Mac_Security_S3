...
//
// Blackosx - Feb 2013 -> November 2013
//
//
...
$(document).ready(function() {    
    ...
    // Set the colour of the htmlReport menu depending on the
    // selected value.
    setColourHtmlReport();
...
$(function()
{
    ...
    // On changing the 'htmlReport' dropdown menu.
    $("#htmlReport").change(function() {
        setColourHtmlReport();
    });
...
//-------------------------------------------------------------------------------------
// Change colour of the htmlReport drop down menu text
// depending on the selected value.
// Note: these colours match the values used in .css
function setColourHtmlReport()
{
    var temp=$("#htmlReport").val(); 
    if ( temp=="ReportNone" ) {
        $("#htmlReport").css("color","#666");
    } else {
        $("#htmlReport").css("color","#FFF"); // 68CF44
    }
} 
...
function readLastSettings()
{
        ...
        if (prevSettings != "") {
            ...
            for (var i = 0; i < prevSettings.length; i++) {
                ...
                if (prevSettings[i] != "ArchiveZip" && prevSettings[i] != "ArchiveLzma" && prevSettings[i] != "ArchiveNone" && prevSettings[i] != "ReportNone" && prevSettings[i] != "ReportNew" && prevSettings[i] != "ReportClassicCollapsed" && prevSettings[i] != "ReportClassic") {
                    ...
                    if (prevSettings[i] == "ReportNone" || prevSettings[i] == "ReportNew" || prevSettings[i] == "ReportClassicCollapsed" || prevSettings[i] == "ReportClassic") {
                        // Set the htmlReport drop down menu.
                        $('[name=htmlReportOptions]').val(prevSettings[i]);
                    }
                }
            }
        }
    ...
    { // There is no prefs file.
        ...
        // Check the enableHTML and collapsetables checkboxes
        //$("#enablehtml").prop('checked', true);
        //$("#collapsetables").prop('checked', true);
        
        // Set the htmlReport drop down menu to ReportNew
        $('[name=htmlReportOptions]').val("ReportNew");
...
{ 
    ...
    // add to string user choice of htmlReport
    string=(string+","+$("#htmlReport").val())
    ...
    if($("#htmlReport").val()=="ReportNew" || $("#htmlReport").val()=="ReportClassicCollapsed" || $("#htmlReport").val()=="ReportClassic")
        $("#status_htmlReport").attr('class', 'statusBarWaiting');
            ...
            for (var i = 0; i < optionsFedBackFromScriptArray.length; i++) {
                ...
                if (optionsFedBackFromScriptArray[i] != "done") {
                   ...
                   if (optionsFedBackFromScriptArray[i] == "X") {
                       ...
                       $("#htmlReport").prop("disabled", false);
    ...
    if (userPath != 0 && appPath != 0) {
        ...
        if (userPath[0] != invalidPath) {
            ...
            if (userPath[0] != appPath[0] && userPath[0] != appPath[0]+"/DarwinDumperReports") {
                ...
                //var cola = $("#info_saveDirectory").css("color");
                //if ( cola == "rgb(255, 0, 0)" ) {
                //    //alert("A1: read RED");
                //    $("#info_saveDirectory").css("color","#FFFFFF");
                //}
            }
            else
            {
