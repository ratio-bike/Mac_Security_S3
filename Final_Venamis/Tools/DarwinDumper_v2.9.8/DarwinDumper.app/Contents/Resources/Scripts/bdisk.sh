...
#createHtml    # This is now set by passing the 'html' argument when calling script. This automatically sets writeFiles to 1 also.
...
#writeFiles    # This is now set by passing argument when calling script. This is automatically set when passing 'html' argument to script.
...
# Save Location
if [ "$1" == "" ]; then
    dumpFolderPath="$( cd -P "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
else
    dumpFolderPath="$1"
fi 
#gBootSectDir="$dumpFolderPath"/"BootSectDump"
...
if [ "$diskToRead" == "" ]; then
    ...
    echo "Please use: ./bdisk.sh diskX"
    exit 1
fi
            ...
            if [ $signature == "aa55" ]; then
                ...
                if [ $signature == "0000" ] && [ $passedSector -eq 0 ]; then
                    ...
                    if [ $apmSignature == "504d" ]; then
                        ...
                        echo "          Found Apple Partition Map at ${diskToRead}. Skipping." >> "${gLogFile}"
...
echo "          Scanning & interpreting partition table for ${diskToRead}" >> "${gLogFile}"
...
# Set temporary HTML build file to parse later.
if [ $createHtml -eq 1 ]; then
    gTmpHtmlParseFile="$gDDTmpFolder"/html_build_file_${diskToRead}.txt
fi
